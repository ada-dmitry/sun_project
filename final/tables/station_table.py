    ...
    def insert(self):
        while True:
            ...
            if len(st_name) > 20:
                ...
                break
        tarrif_zone_id = add_func.validate_input('Введите номер тарифной зоны: ', 0, 7)
        if(tarrif_zone_id==-1):
            return "1"
        st_index = add_func.validate_input('Введите индекс добавляемой станции: ', 0, 100)
        if(st_index==-1):
            return "1"
    ...
    def update(self, old):
        ...
        while True:
            ...
            if(len(data.strip()) > 20):
                ...
                param_sql = f"UPDATE {self.table_name()} SET st_name = '{data}' WHERE st_name = '{old[1]}';"
